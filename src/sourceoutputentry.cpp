...
void SourceOutputEntry::setVolume(PAInterface *interface, const int channel, const pa_volume_t volume)
{
	...
	pa_operation *op = pa_context_set_source_output_volume(interface->getPAContext(), m_Index, cvol, &PAInterface::cb_success, interface);
...
void SourceOutputEntry::cycleSwitch(PAInterface *interface, bool increment)
{
	...
	pa_operation *op = pa_context_move_source_output_by_index(interface->getPAContext(), m_Index, source->second->m_Index, &PAInterface::cb_success, interface);
	while (pa_operation_get_state(op) == PA_OPERATION_RUNNING)
		pa_threaded_mainloop_wait(interface->getPAMainloop());
	pa_operation_unref(op);
}
