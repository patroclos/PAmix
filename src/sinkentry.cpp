...
void SinkEntry::setVolume(const int channel, const pa_volume_t volume) {
	...
	pa_operation *op = pa_context_set_sink_volume_by_index(interface->getPAContext(), m_Index, cvol,
...
void SinkEntry::setMute(bool mute) {
	...
	pa_operation *op = pa_context_set_sink_mute_by_index(interface->getPAContext(), m_Index, mute,
	                                                     &PAInterface::cb_success, interface);
	while (pa_operation_get_state(op) == PA_OPERATION_RUNNING)
		pa_threaded_mainloop_wait(interface->getPAMainloop());
	pa_operation_unref(op);
...
void SinkEntry::setPort(const char *port) {
	...
	pa_operation *op = pa_context_set_sink_port_by_index(interface->getPAContext(), m_Index, port,
	                                                     PAInterface::cb_success, interface);
